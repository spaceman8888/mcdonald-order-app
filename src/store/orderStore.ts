...
interface OrderState {
  ...
  export const useOrderStore = create<OrderState>((set, get) => ({
    ...
    initializeSession: () => {
      ...
      set({
        ...
        chatMessages: [
          {
            role: 'assistant',
            content: '안녕하세요! 맥도날드 주문을 도와드리겠습니다. 어떤 메뉴를 원하시나요?'
          }
        ]
      });
    },
    ...
    addToCart: async (menuItem: MenuItem) => {
      try {
        ...
        const existingItemIndex = cartItems.findIndex(item => 
          item.id === menuItem.id && item.options.length === 0
        );
    ...
    addItemToCartById: async (menuId: number, quantity: number = 1, optionIds: number[] = []) => {
      try {
        const details = await menuService.getMenuItemDetails(menuId);
        
        // 선택한 옵션 정보 수집
        let selectedOptions: {id:number, name:string, price_adjustment:number}[] = [];
    ...
    completeOrder: async () => {
      ...
      try {
        ...
        // 장바구니 세션 저장
        await orderService.saveCartSession(sessionId, cartItems);
